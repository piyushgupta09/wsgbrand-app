<template>
    <a id="floatingButton" ref="floatingButton" class="btn btn-dark rounded-circle" @mousedown="handleMouseDown"
        @touchstart="handleTouchStart" @touchmove="handleTouchMove" @touchend="handleTouchEnd" @click="handleButtonClick"
        :style="{
            opacity: buttonOpacity,
            top: storedPosition.top + 'px',
            left: storedPosition.left + 'px'
        }"
        :href="`/${chatPath}/${ledger}`">
...
export default {
    ...
    data() {
        return {
            ...
            storedPosition: { bottom: 0, left: 0 } // Default position
        };
    },
    computed: {
        collapseLeft() {
    ...
    mounted() {
        this.ledger = this.$route.params.ledger;
        console.log(this.ledger + " from chat button " + this.chatPath)
