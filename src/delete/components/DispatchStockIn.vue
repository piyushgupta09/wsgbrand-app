        ...
        <div class="offcanvas-body pt-3" style="overflow-x: hidden;">
            ...
            <form @submit.prevent="postPurchase" method="post" class="w-100 h-100" style="overflow-x: hidden;">
                ...
                <div class="d-flex justify-content-between mb-3" v-for="(item, itemIndex) in activeDispatch.items"
                    ...
                    <img :src="item.image" class="of-cover w-100p" data-bs-toggle="modal" data-bs-target="#ledgerImageModal"
...
export default {
    ...
    watch: {
        activeDispatch: {
            ...
            handler(newValue, oldValue) {
                console.log('activeDispatch watcher');
    ...
    methods: {
        ...
        increment(item) {
            if (item.billed_qty < Math.round(item.max_quantity)) {
                item.billed_qty++;
            } else {
                this.error = 'Maximum quantity reached.';
                item.billed_qty = Math.round(item.max_quantity);
            }
        },
        decrement(item) {
            if (item.billed_qty > Math.round(item.min_quantity)) {
                item.billed_qty--;
            } else {
                this.error = 'Minimum quantity reached.';
                item.billed_qty = Math.round(item.min_quantity);
            }
        },
        postPurchase() {
